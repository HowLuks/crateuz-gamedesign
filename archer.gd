...
@onready var animation: AnimationPlayer = get_node("Bow/Animation")
...
func _ready():
	# Garante que o sinal está conectado corretamente
...
func spawn_arrow() -> void:
	...
	arrow.global_position = global_position + (last_direction * offset_distance)  
...
func _on_animation_finished(anim_name: StringName) -> void:
	if anim_name == "attack":  # Confirma que a animação terminada é a de ataque
		is_attacking = false  # Agora pode atacar novamente
